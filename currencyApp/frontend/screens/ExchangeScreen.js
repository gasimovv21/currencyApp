...
const ExchangeScreen = ({ route, navigation }) => {
    ...
    const fetchExchangeRates = async () => {
      try {
        ...
        const ratePromises = currencyCodes.map((currency) =>
          fetch(
            `https://api.nbp.pl/api/exchangerates/rates/c/${currency.toLowerCase()}/?format=json`
          )
        );
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {userCurrencies.filter((currency) => currency.currency_code !== "PLN")
          .length === 0 ? (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>
              You don't have an account other than PLN account.
            </Text>
            <Text style={styles.message}>
              If you would like to exchange money, you must create at least one
              account.
            </Text>
          </View>
        ) : (
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      navigateToOperation(
                        ...
                        "bid",
                        "SELL",
                        String(exchangeRates[currency.currency_code]?.["bid"])
                      )
                    }
                  >
                    <Text style={styles.buttonText}>
                      SELL {currency.currency_code}
                    </Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      navigateToOperation(
                        ...
                        "ask",
                        "BUY",
                        String(exchangeRates[currency.currency_code]?.["ask"])
                      )
                    }
                  >
                    <Text style={styles.buttonText}>
                      BUY {currency.currency_code}
                    </Text>
                  </TouchableOpacity>
                </View>
                <View style={styles.rateContainer}>
                  {exchangeRates[currency.currency_code]?.error ? (
                    <Text style={styles.rate}>
                      Error: {exchangeRates[currency.currency_code].error}
                    </Text>
                  ) : (
                    <>
                      <Text style={styles.rate}>
                        Bid:{" "}
                        {exchangeRates[currency.currency_code]?.bid || "N/A"}
                      </Text>
                      <Text style={styles.rate}>
                        Ask:{" "}
                        {exchangeRates[currency.currency_code]?.ask || "N/A"}
                      </Text>
                    </>
                  )}
                </View>
